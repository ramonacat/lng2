...
use crate::{
    ADDRESS_SPACE,
    codegen::CompiledClass,
    identifier::Identifier,
    module::{CompilerServices, ModuleCompiler},
};

#[derive(Clone)]
pub struct ClassValue<'ctx> {
    fields: HashMap<Identifier, Value<'ctx>>,
}

impl<'ctx> ClassValue<'ctx> {
    pub(crate) fn new(class: &CompiledClass<'ctx>) -> Self {
        Self {
            fields: class.fields.clone(),
        }
    }

    pub(crate) fn field(&self, field: Identifier) -> &Value<'ctx> {
...
impl<'ctx> ObjectFunctions<'ctx> {
    pub(crate) fn declare_class(
        ...
        let fields_field = builder
            ...
            .unwrap();
        let fields_value =
            module.add_global(fields_type.array_type(fields.len() as u32), None, "fields");
        fields_value.set_initializer(&fields_type.array_type(fields.len() as u32).const_zero());
        ...
        for (index, field) in fields.iter().enumerate() {
            ...
            builder
                .build_store(
                    ...
                    context
                        .i64_type()
                        .const_int(field.name.as_id() as u64, false),
                )
                .unwrap();
